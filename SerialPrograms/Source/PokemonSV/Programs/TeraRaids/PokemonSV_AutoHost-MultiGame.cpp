...
/*
void AutoHostMultiGame::switch_game(SingleSwitchProgramEnvironment& env, BotBaseContext& context, uint8_t g){
    AutoHostMultiGame_Descriptor::Stats& stats = env.current_stats<AutoHostMultiGame_Descriptor::Stats>();
    ...
    if (game_slot == g) {
        ...
        pbf_press_button(context, BUTTON_A, 10, 10);
        if (gamemenu_to_ingame(env.console, context)) {
            env.log("Started Next Profile", COLOR_BLUE);
        }
    }
    ...
    pbf_wait(context, game.pre_raid_delay);
}
 */
